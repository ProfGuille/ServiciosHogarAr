...
export function ReferralShareCard() {
  ...
  const shareViaWhatsApp = () => {
    const message = `¡Te invito a unirte a ServiciosHogar! Encontrá profesionales del hogar de confianza en tu zona: ${shareUrl}`;
    window.open(`https://wa.me/?text=${encodeURIComponent(message)}`, "_blank");
  };

  const shareViaEmail = () => {
    const subject = "Invitación a ServiciosHogar";
    const body = `¡Hola!\n\nTe invito a unirte a ServiciosHogar, una plataforma para encontrar profesionales de confianza.\n\nRegistrate con mi enlace y encontrá profesionales del hogar de confianza: ${shareUrl}\n\n¡Nos vemos allí!`;
            ...
            {user?.userType === 'provider' ? (
              <>
                <li className="flex items-start gap-2">
                  ...
                  <span className="text-sm">Ganás 1 crédito cuando tu referido se registra en la plataforma</span>
                </li>
                <li className="flex items-start gap-2">
                  <CheckCircle className="h-4 w-4 text-green-500 mt-0.5 flex-shrink-0" />
                  <span className="text-sm">Ganás 1 crédito adicional cuando tu referido realiza su primera compra de créditos</span>
                </li>
              </>
            ) : (
              <>
