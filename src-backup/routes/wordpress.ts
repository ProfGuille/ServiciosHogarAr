...
export function registerWordPressRoutes(app: Express) {
  ...
  app.post("/api/admin/wordpress/api-keys", isAuthenticated, async (req: any, res) => {
    try {
  ...
  // Admin: List API Keys
  app.get("/api/admin/wordpress/api-keys", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const user = await db.select().from(users).where(eq(users.id, userId));
  ...
  app.get("/api/wordpress/categories", validateWordPressKey, async (req, res) => {
    try {
  ...
  app.get("/api/wordpress/providers", validateWordPressKey, async (req, res) => {
    try {
  ...
  app.post("/api/wordpress/content/sync", validateWordPressKey, async (req, res) => {
    try {
  ...
  app.get("/api/seo/:pageType/:identifier?", async (req, res) => {
    try {
  ...
  app.get("/api/wordpress/sync-logs", validateWordPressKey, async (req, res) => {
    try {
