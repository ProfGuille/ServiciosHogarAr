...
import { registerRoutes } from "./routes/index";  // Con extensión  si usás moduleResolution nodo moderno
...
app.use((req, res, next) => {
  ...
  const originalResJson = reson;
  reson = function (bodyJson, ...args) {
...
async function initializeDefaultCategories() {
  ...
  if (categories.length === 0) {
    log("Initializing default service categories...");
...
(async () => {
  ...
  app.get('/api/health', (_req, res) => {
    reson({ status: 'ok', timestamp: new Date().toISOString() });
  });
  ...
  // Si usás Vite para frontend, activá acá la función setupVite importada
  /*
  if (app.get("env") === "development") {
    await setupVite(app, server);
  } else {
    app.use(serveStatic("public")); // O la carpeta que uses para frontend compilado
  }
  */
  ...
  server.listen(port, "0.0.0.0", () => {
    ...
    log(`External URL: https://${process.env.REPL_SLUG || "localhost"}.${process.env.REPL_OWNER || "localhost"}.repl.co`);
  });
})();
