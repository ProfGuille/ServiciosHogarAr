...
export class SearchService {
  ...
  static async searchProviders(filters: SearchFilters): Promise<SearchResult> {
    ...
    switch (filters.sortBy) {
      ...
      case 'response_time':
        ...
        break;
      default: // relevance
        // Sort by multiple factors for relevance
        query = query.orderBy(
  ...
  private static async getFacets(baseConditions: any[]) {
    ...
    return {
      ...
      cities: cityFacets.filter(c => c.name !== null) as { name: string; count: number }[],
  ...
  static async getSearchSuggestions(query: string): Promise<string[]> {
    ...
    businesses.forEach(b => b.name && suggestions.add(b.name));
